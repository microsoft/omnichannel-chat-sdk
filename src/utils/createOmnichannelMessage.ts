...
const createOmnichannelMessage = (message: IRawMessage | ChatMessageReceivedEvent | ChatMessageEditedEvent | ChatMessage, optionalParams: CreateOmnichannelMessageOptionalParams): OmnichannelMessage => {
    ...
    console.warn("LOPEZ :: Message  => ", message);
    ...
    if (metadata) {
        const citationKeys = ["Text", "pva:gpt-feedback", "feedbackLoop", "deliveryMode"];
        const hasAllCitationKeys = citationKeys.every(key => key in metadata);
        if (hasAllCitationKeys) {
