---
description: Local development, packaging, and end-to-end test workflow across the four LCW repos in c:\lcw (omnichannel-sdk, omnichannel-chat-sdk, omnichannel-chat-widget, CRM.OmniChannel.LiveChatWidget). Use when changing code in any of these repos and validating the change in the live chat widget served at http://localhost:9000.
applyTo: "**"
---
...
- `omnichannel-chat-sdk` is symlinked into chat-widget and LCW via `yarn link`. Check with:
  ```bash
  ...
  ```
  Both should point to `~/AppData/Local/Yarn/Data/link/@microsoft/omnichannel-chat-sdk/`.
...
1. **ACS adapter resolves to `0.0.1-beta-1` instead of `beta.N`** — caret ranges (`^0.0.1-beta.6` etc.) resolve to the rogue prerelease `0.0.1-beta-1` per semver §11 (single pre-release identifier `beta-1` sorts higher than dot-separated `beta.N`). Always **pin the exact version** (`"0.0.1-beta.8"`, no caret) and verify with:
   ```bash
